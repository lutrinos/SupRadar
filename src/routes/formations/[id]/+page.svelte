...
<svelte:head>
	<title>{data.formation.nom} | SupRadar</title>
	<meta
		name="description"
		content={`Informations de la formation ${data.formation.nom} à ${data.formation.commune}`}
	/>
</svelte:head>
			...
			<div class="space-y-6">
				...
				<div class="card card-border bg-base-100">
					<div class="card-body relative">
						...
						<Info>Pas encore implémenté.</Info>
						<h2 class="card-title text-primary">
							Établissements proposant cette formation
						</h2>
						<div class="space-y-2">
							Bientôt ! Une liste est disponible <a
								class="link"
								href={`/filieres/${getFiliere(1, data.formation.filiere1)?.slug}/${getFiliere(2, data.formation.filiere2)?.slug}/${getFiliere(3, data.formation.filiere3)?.slug}`}
								>ici</a
							>.
						</div>
					</div>
				...
				<div class="card card-border bg-base-100">
					<div class="card-body">
						<Info>
							Une liste des différentes formations proposées par
							l'établissement.
						</Info>

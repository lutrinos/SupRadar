<script lang="ts">
    ...
    let isValid = (filiere3: any) => {
        // @ts-ignore
        return filieres2[filiere3.parent.toString()]?.slug === params.id2;
    };
...
<div class="flex justify-center min-h-screen bg-base-100 py-12">
    <div class="container">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {#each Object.values(filieres3) as f}
                {#if isValid(f)}
                    <a
                        href={`/filieres/${params.id1}/${params.id2}/${f.slug}`}
